...
var gatesim = (function() {
	...
	var ignored_components = ["analog:port-in",
				  "analog:port-out",
				  "analog:port-inout",
				  "analog:s"
				  ];
		...
		while (this.time < this.tstop && this.event_queue != null) {
			...
			setTimeout(function() {
				try {
				    ...
				    if (typeof e == 'string')
					nl.progress.finish(e,nl.progress);
				    else throw e;
				}
			    },1);
	...
	Node.prototype.add_fanout = function(device) {
	    if (this.fanouts.indexOf(device) == -1)
		this.fanouts.push(device);
	};
	...
	Node.prototype.finalize = function() {
	    if (this.drivers === undefined || this.driver !== undefined)
		...
		this.capacitance = c_intercept + c_slope*(ndrivers + nfanouts);
	...
	var LTable = []; LTable.push(LTable,LTable,LTable,LTable,0);  // always "0"
	var HTable = []; HTable.push(HTable,HTable,HTable,HTable,1);  // always "1"
	var XTable = []; XTable.push(XTable,XTable,XTable,XTable,2);  // always "X"
	var ZTable = []; ZTable.push(ZTable,ZTable,ZTable,ZTable,3);  // always "Z"
	...
	// and tables
	var AndXTable = []; AndXTable.push(LTable,AndXTable,AndXTable,AndXTable,2);
	var AndTable = []; AndTable.push(LTable,AndTable,AndXTable,AndXTable,1);

	// nand tables
	var NandXTable = []; NandXTable.push(HTable,NandXTable,NandXTable,NandXTable,2);
	var NandTable = []; NandTable.push(HTable,NandTable,NandXTable,NandXTable,0);

	// or tables
	var OrXTable = []; OrXTable.push(OrXTable,HTable,OrXTable,OrXTable,2);
	var OrTable = []; OrTable.push(OrTable,HTable,OrXTable,OrXTable,0);

	// nor tables
	var NorXTable = []; NorXTable.push(NorXTable,LTable,NorXTable,NorXTable,2);
	var NorTable = []; NorTable.push(NorTable,LTable,NorXTable,NorXTable,1);
	...
	function LogicGate(network,type,name,table,inputs,output,properties,lenient) {
	    ...
	    var in5 = inputs[5];
	    if (inputs.length == 0)
		this.eval = function() { return table[4]; };;
	    else if (inputs.length == 1)
		this.eval = function() { return table[in0.v][4]; };;
	    else if (inputs.length == 2)
		this.eval = function() { return table[in0.v][in1.v][4]; };;
	    else if (inputs.length == 3)
		this.eval = function() { return table[in0.v][in1.v][in2.v][4]; };;
	    else if (inputs.length == 4)
		this.eval = function() { return table[in0.v][in1.v][in2.v][in3.v][4]; };;
	    else if (inputs.length == 5)
		this.eval = function() { return table[in0.v][in1.v][in2.v][in3.v][in4.v][4]; };;
	    else if (inputs.length == 6)
		this.eval = function() { return table[in0.v][in1.v][in2.v][in3.v][in4.v][in5.v][4]; };;
