...
jade = (function() {
	...
	Library.prototype.save = function() {
		...
		var args = {
		    url: 'server.cgi',
		    type: 'POST',
		    data: {file: this.name, json: JSON.stringify(this.json())},
	...
	Aspect.prototype.end_action = function() {
		...
		if (this.actions.length > this.current_action)
		    this.actions = this.actions.slice(0,this.current_action);
	...
	Component.prototype.edit_properties = function(diagram,x,y,callback) {
		...
		for (var p in this.properties) {
		    ...
		    var input;
		    if (mprop.type == 'menu')
			input = build_select(mprop.choices,this.properties[p]);
		    else {
			var v = this.properties[p];
	...
	*/
	...
	function diagram_copy(diagram) {
	    ...
	    diagram.aspect.map_over_components(function(c) {
		    if (c.selected)
			clipboards[diagram.editor.editor_name].push(c.clone(c.coords[0],c.coords[1]));
		});
	...
	Diagram.prototype.pan_zoom = function() {
		...
		if (Math.abs(sy) > Math.abs(sx)) {   // N or S
		    var delta = this.canvas.height / (8*this.scale);
		    if (sy > 0) delta = -delta;
		    var temp = this.origin_y - delta;
		    if (temp > this.origin_min*this.grid && temp < this.origin_max*this.grid) this.origin_y = temp;
		} else {			    // E or W
		    var delta = this.canvas.width / (8*this.scale);
		    if (sx < 0) delta = -delta;
		    var temp = this.origin_x + delta;
	...
	function dialog_okay(event) {
	    ...
	    if (dialog.callback) 
		setTimeout(function(){ dialog.callback(); },1);
	}
	...
	*/
	...
	function Jade(owner) {
	    ...
	    $(e).keypress(function(event) {
		    ...
		    if (event.keyCode == 13)
			owner.jade.edit(event.target.value);
		});
	...
	var dont_print = {'analog:g':true,'analog:vdd':true,'analog:s':true,'analog:port-in':true,'analog:port-out':true};
	...
	function Schematic(div,jade) {
	    ...
	    this.toolbar.add_tool('undo',undo_icon,'Undo: undo effect of previous action',diagram_undo,
				  function(diagram){ return diagram.aspect.can_undo(); });
	    this.toolbar.add_tool('redo',redo_icon,'redo: redo effect of next action',diagram_redo,
				  function(diagram){ return diagram.aspect.can_redo(); });
	    function has_selections(diagram) { return diagram.aspect.selections(); };
	    this.toolbar.add_tool('cut',cut_icon,'Cut: move selected components from diagram to the clipboard',diagram_cut,
				  has_selections);
	    this.toolbar.add_tool('copy',copy_icon,'Copy: copy selected components into the clipboard',diagram_copy,
				  has_selections);
	    this.toolbar.add_tool('paste',paste_icon,'Paste: copy clipboard into the diagram',diagram_paste,
				  function(diagram){ return clipboards[diagram.editor.editor_name].length > 0; });
	    this.toolbar.add_tool('fliph',fliph_icon,'Flip Horizontally: flip selection horizontally',diagram_fliph,
				  has_selections);
	    this.toolbar.add_tool('flipv',flipv_icon,'Flip Vertically: flip selection vertically',diagram_flipv,
				  has_selections);
	    this.toolbar.add_tool('rotcw',rotcw_icon,'Rotate Clockwise: rotate selection clockwise',diagram_rotcw,
				  has_selections);
	    this.toolbar.add_tool('rotccw',rotccw_icon,'Rotate Counterclockwise: rotate selection counterclockwise',diagram_rotccw,
				  has_selections);
	    ...
	    this.toolbar.add_tool('up',up_icon,'Up in the hierarchy: return to including module',schematic_up,
				  function(diagram){ return diagram.editor.hierarchy_stack.length > 0});
	...
	function schematic_mouse_down(event) {
	    ...
	    if (dx <= connection_point_radius && dy <= connection_point_radius &&
	...
	var wire_module = {properties: {
		"signal": {"type":"string","label":"Signal name","value":"","edit":"yes"}
	    }};
	...
	Wire.prototype.bisect_cp = function(cp) {
	    ...
	    if (this.inside(x,y) &&
		this.distance(x,y) < 1 &&
		!this.connections[0].coincident(x,y) &&
	...
	Icon.prototype.set_mode = function(mode) {
	    ...
	    if (mode == 'select')
		this.diagram.canvas.style.cursor = 'auto';
	    else
	...
	function icon_arc(diagram) { diagram.editor.set_mode('arc'); }
	function icon_circle(diagram) { diagram.editor.set_mode('circle'); }
	function icon_text(diagram) { diagram.editor.set_mode('text'); }
	function icon_terminal(diagram) { diagram.editor.set_mode('terminal'); }
	function icon_property(diagram) { diagram.editor.set_mode('property'); }
	...
	function text_bbox(text,align) {
	    ...
	    if (vertical == 'top') { bbox[1] = 0; bbox[3] = h; }
	    else if (vertical == 'center') { bbox[1] = -h/2; bbox[3] = h/2; }
	    else { bbox[1] = -h; bbox[3] = 0; }

	    var horizontal = position[1] || position[0];
	    if (horizontal == 'left') { bbox[0] = 0; bbox[2] = w; }
	    else if (horizontal == 'center') { bbox[0] = -w/2; bbox[2] = w/2; }
	    else { bbox[0] = -w; bbox[2] = 0; }
	...
	var text_module = {properties: {
		"text": {"type":"string","label":"Text","value":"???","edit":"yes"},
		"font": {"type":"string","label":"CSS Font","value":"6pt sans-serif","edit":"yes"},
		"align": {"type":"menu","label":"Alignment","value":"center-left","edit":"yes",
	...
	var property_module = {properties: {
		"format": {"type":"string","label":"Format","value":"{???}","edit":"yes"},
		"align": {"type":"menu","label":"Alignment","value":"center-left","edit":"yes",
			  "choices":text_alignments}
	    }};
	...
	var terminal_module = {properties: {
		"name": {"type":"string","label":"Terminal name","value":"???","edit":"yes"},
		"line": {"type":"menu","label":"Draw line","value":"yes","edit":"yes","choices":["yes","no"]}
	    }};
	...
	PropertyEditor.prototype.build_table = function() {
	    ...
	    fields.action = build_button('add',function(event) {
		    ...
		    else if (name in editor.module.properties)
